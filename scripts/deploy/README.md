...
本仓库提供 **self-contained 二进制** 部署方案（Docker 方案已废弃移除）：

| 方案 | 适用场景 | 部署产物 | 文件 |
|---|---|---|---|
| **self-contained 二进制（云服务器）** | Ubuntu 云服务器 | GitHub Release 的 `.tar.gz` | `bootstrap.sh` + `deploy.sh` + `childnotes-api.service` |

> 2026-09 起 Docker 部署方案（docker-compose.yml / Dockerfile / docker-publish.yml / nas-* 脚本）已全部移除，统一走 systemd + self-contained 二进制部署。

## 部署方案：self-contained 二进制部署（云服务器）
...
```bash
# 测试 PostgreSQL 连接
PGPASSWORD='(生产配置中的数据库密码)' psql -U childnotes -h 127.0.0.1 -d child_notes -c "\q"

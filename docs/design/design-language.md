...
产品首先是一个记录和回忆产品，AI 是其中一个重要能力，不是产品的中心。所有设计决策应遵循以下优先级顺序：
...
不要让产品从"一本家庭成长日记，带 AI 辅助"滑向"一个 AI 育儿产品，顺便记录"。AI 的篇幅、视觉权重、入口数量都应服务于这个层级，而不是反过来。
...
AI 是记录助手、记忆整理者和温和陪伴者，不是产品的中心，只在能降低记录成本或提升理解时出现。核心边界：

- 原始记录永远属于用户，AI 输出必须可确认、可修改、可拒绝，永远不能绕过用户直接修改记忆。
- AI 不替代父母表达、不生成冰冷分析报告、不制造焦虑、不进行医疗诊断；建议应温和、解释充分。
...
AI 体验围绕孩子成长、家庭记忆和历史记录，不是通用 AI 助手。三个要点：

- **人优先**：输出可修改、可确认、可拒绝（已在产品原则中明确）。
- **可理解**：不只给结论。错误示例："宝宝语言能力很好"；推荐："根据最近三个月记录，宝宝出现了更多主动发音和模仿行为"。
- **有温度**：回复像家庭助手而非客服机器人；等待过程也属于体验——温和提示当前状态（如"正在整理宝宝最近的成长记录..."），而不是冰冷的"Loading..."。

涉及儿童信息时：用户控制数据、AI 边界清晰、不制造焦虑、不进行医疗诊断。AI 组件的具体结构与状态见 [`components.md`](components.md)。

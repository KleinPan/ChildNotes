...
- 12 小时制时间解析（重要，按当前时段推断 AM/PM）：
  ...
  - 用户未说时段词时（如"5点吃了奶"、"八点半睡觉"）→ 按当前实际时间推断：
    * 若当前是上午（当前小时 0~11）→ 按当前时段 AM 解析（5点=05:00）
    * 若当前是下午/晚上/夜里（当前小时 12~23）→ 按当前时段 PM 解析（5点=17:00、8点=20:00）
    * 当前时间见上方"当前日期时间"
  - 不要基于"吃奶/睡觉"等场景语义推断 AM/PM，只依据：①显式时段词 ②当前实际时间
  - 示例（当前为下午 15:00 时）："五点半吃了奶粉" → time="17:30"（无时段词，按当前 PM）
  - 示例（当前为上午 09:00 时）："五点半吃了奶粉" → time="05:30"（无时段词，按当前 AM）
  - 示例（当前为下午 15:00 时）："早上五点半吃了奶粉" → time="05:30"（显式"早上"，强制 AM）

...
public sealed class AiParseApiClient : BaseApiClient
{
    ...
    /// 通过 URL 查询参数传递（后端 Controller 为 [FromQuery] 绑定，放 body 不生效）。
    /// </summary>
    ...
    {
        var path = BuildParsePath(usePointsForOverage);
    ...
    /// <paramref name="parseMode"/> 见 <see cref="ParseMode"/>；默认 <see cref="ParseMode.Fast"/>。
    /// <paramref name="usePointsForOverage"/> 为 true 时通过 URL 查询参数传递（积分抵扣模式）。
    /// </summary>
    ...
    /// <summary>
    /// 构造 /parse-note 请求路径：积分抵扣模式时追加 usePointsForOverage=true 查询参数
    /// （后端 Controller 用 [FromQuery] 绑定，放请求体不生效）。
    /// </summary>
    private static string BuildParsePath(bool usePointsForOverage)
        => usePointsForOverage ? "/api/smart-analysis/parse-note?usePointsForOverage=true" : "/api/smart-analysis/parse-note";
}

...
public sealed partial class RecordDisplayItem : ObservableObject
{
    ...
    /// sub 显示分类标签（主食/蔬菜/水果/肉蛋/其他）+ 量，
    /// note 仅显示用户手填备注。
    ///
    /// 分类标签来源优先级：
    /// 1. dto.FoodTypes 已是分类标签（AI 解析输出）→ 直接使用
    /// 2. dto.FoodTypes 是具体食材名（表单保存的旧数据）→ 通过 FoodCategoryMapper 转为分类标签
    /// 3. dto.FoodTypes 为空 → 从 dto.FoodName 推断分类标签
    /// </summary>
    private static (string Title, string Sub, string Extra, string Note) BuildComplementaryText(ChildRecord r)
    {
        ...
        // 1) FoodTypes 可能已是分类标签（AI 解析输出如 ["主食","肉蛋"]）
        //    也可能是具体食材名（表单保存如 ["米粉","菠菜"]），需转译
        if (dto?.FoodTypes.Count > 0)
        {
            foreach (var ft in dto!.FoodTypes)
            {
                // 判断是否已为分类标签
                var isCategory = ft is "主食" or "蔬菜" or "水果" or "肉蛋" or "其他";
                if (isCategory)
                    categories.Add(ft);
                else
                {
                    // 具体食材名 → 转为分类标签
                    var cat = FoodCategoryMapper.GetCategory(ft);
                    if (cat is not null) categories.Add(cat);
                }
            }
        }

        // 2) 若 FoodTypes 未提供足够分类信息，从 FoodName 补充推断
        if (categories.Count == 0 && !string.IsNullOrWhiteSpace(dto?.FoodName))
        {
            foreach (var cat in FoodCategoryMapper.ExtractCategories(dto!.FoodName))
                categories.Add(cat);
        }
